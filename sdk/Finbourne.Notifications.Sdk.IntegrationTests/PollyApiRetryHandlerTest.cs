...
namespace Finbourne.Notifications.Sdk.IntegrationTests
{
    ...
    public class PollyApiRetryHandlerTests
    {
        ...
        private readonly AsyncPolicy<HttpResponseMessage> _initialRetryPolicy = RetryConfiguration.AsyncRetryPolicy;
        ...
        private readonly Notification _mockResponse = new Notification("id",
            "displayName",
            "description",
            new NotificationNotificationType(
                new SmsNotificationType(
                    "Sms",
                    "Some SMS Message",
                    new List<string> { "+447700000000" })),
        ...
        public void SetUp()
        {
            ...
            testApiConfig.BaseUrl = ListenerUriPrefix;
            ...
            int returnedStatusCode)
        {
            ...
            response = _apiFactory.Api<INotificationsApi>().GetNotification("no_scope", "no_code", "no_id");
        ...
        public void CallApiMethod_WhenApiResponseStatusCodeSatisfiesRetryCriteria_PollyRetryWithBackoffIsTriggered()
        {
            ...
            RetryConfiguration.AsyncRetryPolicy = Policy<HttpResponseMessage>
                .HandleResult(response => response.StatusCode == (HttpStatusCode)returnedStatusCode)
                .WaitAndRetryAsync(expectedNumberOfRetries,
            ...
            // Calling GetNotification or any other API triggers the flow that triggers polly
            Notification sdkResponse = _apiFactory.Api<INotificationsApi>().GetNotification("no_scope", "no_code", "no_id");
        ...
        public void CallApiMethod_WhenApiResponseStatusCodeSatisfiesRetryCriteria_ConnectivityIssuesNoRetry_Throws()
        {
            ...
            RetryConfiguration.AsyncRetryPolicy = PollyApiRetryHandler.DefaultFallbackPolicyAsync.WrapAsync(
                Policy
                    .HandleResult<HttpResponseMessage>(response => response.StatusCode == 0)
        ...
        public void UsePolicyWrap_WhenCallingApiMethod_BothPoliciesAreUsed()
        {
            ...
            var policy1 = Policy
                .HandleResult<HttpResponseMessage>(apiResponse =>
                    apiResponse.StatusCode == (HttpStatusCode)statusCodeResponse1)
                .RetryAsync(retryCount: 3, onRetry: (result, i) => policy1TriggerCount++);
            var policy2 = Policy
                .HandleResult<HttpResponseMessage>(apiResponse =>
                    apiResponse.StatusCode == (HttpStatusCode)statusCodeResponse2)
                .RetryAsync(retryCount: 3, onRetry: (result, i) => policy2TriggerCount++);
            RetryConfiguration.AsyncRetryPolicy = policy1.WrapAsync(policy2);
        ...
        public void TearDown()
        {
            ...
            RetryConfiguration.AsyncRetryPolicy = _initialRetryPolicy;
